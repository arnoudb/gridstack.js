...
import { GridstackComponent, NgGridStackOptions, NgGridStackWidget, elementCB, gsCreateNgComponents, nodesCB, droppedCB } from 'gridstack/dist/angular';
...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  //
  ///////////////////////////////
  @ViewChild('gridStack2') gridStack2?: GridstackComponent;

  @ViewChild('gridStack1') gridStack1?: GridstackComponent;

  ngAfterViewInit(): void {
    // monkeypatch grid 1
    this.gridStack1?.grid?.on('dropped', (event, previousNode, newNode) => {
      console.log('remove the node');
      this.gridStack1?.grid?.engine.removeNode(newNode);
      //setTimeout(() => {
        console.log('dropped on grid1 is delayed ');
        this.gridStack1?.droppedCB.emit({ event, previousNode, newNode });
      //}, 10);
    });

    // monkeypatch grid 2
    this.gridStack2?.grid?.on('dropped', (event, previousNode, newNode) => {
      console.log('remove the node');
      this.gridStack2?.grid?.engine.removeNode(newNode);
      //setTimeout(() => {
        console.log('dropped on grid2 is delayed ');
        this.gridStack2?.droppedCB.emit({ event, previousNode, newNode });
      //}, 10);
    });
  }

  private draggedItem: GridStackWidget | null = null;
  ...
  protected demo8GridElements2: GridStackWidget[] = [
    {id: 'id_5', x: 4, y: 1, content: '5'},
  ];

  protected demo8DragStart(event: elementCB, grid: "1" | "2") {
    console.log('dragstart', event);

    if (grid === '1') {
      this.draggedItem = this.demo8GridElements1.find(el => el.id === event.el.gridstackNode?.id) ?? null;
    } else if (grid === '2') {
      this.draggedItem = this.demo8GridElements2.find(el => el.id === event.el.gridstackNode?.id) ?? null;
    }

    console.log('dragstart', this.draggedItem);
  }

  protected demo8DroppedCB(event: droppedCB, grid: "1" | "2"): void {
    console.log('dropped', this.draggedItem);

    // update position for re-creation
    this.draggedItem!.x = event.newNode.x;
    this.draggedItem!.y = event.newNode.y;
    this.draggedItem!.w = event.newNode.w;
    this.draggedItem!.h = event.newNode.h;

    if (grid === '1') {
      this.demo8GridElements1 = [...this.demo8GridElements1, this.draggedItem!];
    } else if (grid === '2') {
      this.demo8GridElements2 = [...this.demo8GridElements2, this.draggedItem!];
    }

    this.draggedItem = null
  }

  protected demo8Removed(event: nodesCB, grid: "1" | "2"): void {
    console.log('removed', event);

    if (grid === '1') {
      this.demo8GridElements1 = this.demo8GridElements1.filter(el => el.id !== event.nodes[0].id);
    } else if (grid === '2') {
      this.demo8GridElements2 = this.demo8GridElements2.filter(el => el.id !== event.nodes[0].id);
    }
  }

  protected demo8Added(event: nodesCB, grid: "1" | "2"): void {
